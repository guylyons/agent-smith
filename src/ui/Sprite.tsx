...
// The pixel grid depends only on sessionId+role+name (or an explicit override), and
...
function SpriteImpl({ sessionId, role, name, state, override }: {
  sessionId: string; role: string; name?: string; state: string;
  ...
  const rects = useMemo(() => {
    ...
    const { palette, gear, body } = paletteFor(sessionId, role, name);
    return spriteRects({ body, gear, palette });
  }, [sessionId, role, name, override]);
  return (
